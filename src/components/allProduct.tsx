...
import { handleGetAllArticle } from "../validation/admin/article/handleGet";
...
function AllProduct({ favorites, toggleFavorite, setProductoSeleccionado }: AllProductProps) {
    ...
    return (
        <section id="mejores-productos" className="py-16 bg-white">
            <h2 className="text-4xl font-bold text-center text-[#2F4F4F] mb-10" data-translate>Productos relacionados</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto px-4">
                {topProducts.slice(0, 4).map((product) => (
                    <div key={product.id}
                        className="border border-gray-200 rounded-lg overflow-hidden shadow hover:shadow-lg transition duration-300 relative">
                        <button
