...
function Sold({ favorites, toggleFavorite }: TopProductProps) {
    ...
    useEffect(() => {
        handleGetSelling()
            .then((data: any[]) => {
                const filteredProducts = data.filter((articulo) => Number(articulo.total_vendidos) > 0); // ✅ Filtra solo los vendidos
    ...
    const products: Product[] = articulos.map((articulo) => ({
        ...
        discountPrice: articulo.discount > 0 ? articulo.precio - articulo.discount : articulo.precio,
        images: articulo.imagenes ? String(articulo.imagenes).split(',') : [], 
        sales: 150,
    }));
